...
describe("Log in", () => {
  ...
  it("Is amenities layer loaded and visible?", () => {
    // is layer loaded?
    getStore().its('state').its('map').its('style').its('_layers').should('have.property', "abmAmenities")
    // is layer visible?
    getStore().its('state').its('map').invoke('queryRenderedFeatures').then((yielded) => {
      let containsLayer = (renderedFeature) => renderedFeature.layer.id == "abmAmenities";
  ...
  it("Are abm deck layers loaded?", () => {
    // map.style._layers should include abmDeck Layers
     for (layerName of abmDeckLayers) {
  ...
  it("Are abm trips visible?", () => {
    ...
    getStore().its('state').its('map').its('style').its('_layers').its("abmTrips").its('implementation').its('props').its('visible').should('eq', true)
  })

...
describe("Log in", () => {
  ...
  it("Load a saved noise scenario", () => {
    ...
    getStore().its('state').its('map').its('style').its('_layers').should('have.property', 'noise')
    getStore().its('state').its('map').its('style').its('_layers').should('have.property', 'trafficCounts')
  });
  ...
  it("Is noise layer visible?", () => {
    ...
    getStore().its('state').its('map').invoke('queryRenderedFeatures').then((yielded) => {
      let containsNoiseLayer = (renderedFeature) => renderedFeature.layer.id === "noise";

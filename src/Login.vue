...
export default {
  ...
  methods: {
    ...
    switchVisibility() {
      ...
      this.passwordIcon =
        this.passwordFieldType === "password" ? "mdi-eye" : "mdi-eye-off";
    },
    async submit() {
    ...
    async applyCustomMapSettings() {
      let customMapSettings = await this.$store.state.cityPyO.getLayer(
        "mapSettings",
        false
      );
    ...
    >
      <div>
        <form @submit.prevent="submit" class="submit_form">
          ...
          <label for="password"
            >Password
            <v-icon color="white" @click="switchVisibility">
              {{ passwordIcon }}</v-icon
            ></label
          >
          <input
